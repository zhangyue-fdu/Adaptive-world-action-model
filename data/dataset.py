...
def create_dataset(config: OmegaConf, val: bool = False):
    ...
    if dataset_type == 'robotwin':
        ...
        if hasattr(config.dataset, 'allow_episode_tail_padding'):
            params['allow_episode_tail_padding'] = bool(config.dataset.allow_episode_tail_padding)
        if hasattr(config.dataset, 'robotwin_sampling_mode'):
            rsm = config.dataset.robotwin_sampling_mode
            if rsm is not None and str(rsm).strip():
                params['robotwin_sampling_mode'] = str(rsm).strip()
...
def collate_fn(batch: List[Optional[Dict[str, Any]]]) -> Optional[Dict[str, Any]]:
    ...
    if all(('action_pad_mask' in sample and sample['action_pad_mask'] is not None) for sample in batch):
        result['action_pad_mask'] = torch.stack([sample['action_pad_mask'] for sample in batch])
